...
describe('ProductPrismaRepository 통합 테스트', () => {
  ...
  describe('Stock 동시성 제어 - 100 concurrent requests', () => {
    it('100개의 동시 요청이 들어와도 재고가 정확하게 예약되어야 함', async () => {
      ...
      const requests = Array.from({ length: 100 }, () =>
        repository.reserveStock('test-stock', 1),
      );
      ...
      expect(stock?.reservedQuantity).toBe(100);
      expect(stock?.soldQuantity).toBe(0);
      expect(
    ...
    it('100개의 동시 요청 중 일부는 재고 부족으로 실패해야 함', async () => {
      ...
      expect(stock?.availableQuantity).toBe(0);
      expect(stock?.reservedQuantity).toBe(50);
      expect(stock?.soldQuantity).toBe(0);
    });
    ...
    it('예약-복원-확정이 동시에 발생해도 재고가 정확하게 관리되어야 함', async () => {
      ...
      await Promise.all([...reserveRequests, ...releaseRequests, ...confirmRequests]);
