...
- **처리 과정**:
  ...
  3. 옵션별 추가 가격 조회
- **출력**: 옵션 목록 (type, name, 추가 가격, 재고 상태)
- **예시**:
  - 색상(type): 빨강(name, +0원, 재고있음), 파랑(name, +0원, 재고있음)
  - 사이즈(type): S(name, +0원, 품절), M(name, +0원, 재고있음)
- **관련 유스케이스**: [UC-PROD-02](user-stories.md#uc-prod-02-상품-상세-조회)
...
| **FR-ORDER-02** | 시스템은 주문 생성 시 재고를 확인하고 예약해야 함 | MUST | ⏳ 대기 |
...
- **예약 관리**:
  - availableQuantity: 판매 가능한 재고 (다른 사용자에게 표시)
  - reservedQuantity: 예약된 재고 (해당 주문에만 할당)
  - OrderItem에서 예약 만료 시간 추적 (10분)
- **유지 시간**: 10분 (600초)
- **타임아웃**: 10분 경과 시 배치 작업으로 자동 복원
...
- **상태 목록**:
  - PENDING: 결제 대기 (재고 예약됨)
  - COMPLETED: 결제 완료
  - FAILED: 결제 실패
  - CANCELLED: 주문 취소
  - EXPIRED: 재고 예약 만료 (10분 경과)
- **상태 전이 규칙**:
...
| **FR-PAYMENT-03** | 시스템은 결제 실패/취소/만료 시 예약된 재고를 복원해야 함 | MUST | ⏳ 대기 |
| **FR-PAYMENT-04** | 사용자는 재고 예약 후 10분 이내 재결제를 시도할 수 있어야 함 | MUST | ⏳ 대기 |

#### 상세 설명

**FR-PAYMENT-01: 결제 처리**
- **입력**: 주문 ID, 결제 방법 (CREDIT_CARD)
- **전제 조건**:
  - 결제는 외부 PG사를 통해 처리
  - 주문 생성 시 재고가 이미 예약된 상태
  - 재고 예약 후 10분 이내
- **처리 과정**:
  1. 주문 정보 확인 (PENDING 상태)
  2. OrderItem의 reservationExpiresAt 확인
  3. 만료 시 에러 반환 (재고 예약 만료)
  4. 외부 결제 API 호출 (신용카드 결제)
  5. 결제 기록 저장
...
- **관련 유스케이스**:
  ...
  - [UC-PAYMENT-03](user-stories.md#uc-payment-03-재고-예약-만료)

**FR-PAYMENT-04: 결제 재시도**
- **재시도 조건**: 재고 예약 후 10분 이내
- **처리 과정**:
  1. 주문 상태 확인 (PENDING 또는 FAILED)
  2. OrderItem.reservationExpiresAt 확인 (미래 시간)
  3. 예약 유효 시 결제 재시도 허용
...
- **결제 정확성**: 정확한 결제 처리 보장
- **재고 차감 정확성**: 결제 성공 시 재고 차감 보장 (reserved → sold)
- **재고 복원 보장**: 결제 실패/취소 시 재고 복원 (reserved → available)
- **10분 타임아웃**: 재고 예약 만료 시 자동 복원 (배치 작업)
- **트랜잭션 보장**: ACID 속성 보장
  - Atomicity: 결제 처리와 재고 차감은 원자적 수행
  - Consistency: 데이터 정합성 유지 (totalQuantity = available + reserved + sold)
  - Isolation: 트랜잭션 격리
...
| ID | 요구사항 | 우선순위 | 상태 |
|----|---------|---------|------|
| **FR-USER-01** | 사용자는 본인의 정보를 조회할 수 있어야 함 | MUST | ⏳ 대기 |
| **FR-USER-02** | 시스템은 인증된 사용자만 API에 접근할 수 있도록 제한해야 함 | MUST | ⏳ 대기 |
...
**FR-USER-01: 사용자 정보 조회**
- **입력**: 없음 (인증 토큰에서 사용자 식별)
- **처리 과정**:
  1. 인증 토큰 검증 (Master Token 방식)
  2. 토큰에서 사용자 ID 추출
  3. 사용자 정보 조회
- **출력**: 사용자 정보 (ID, 이름, 이메일, 생성일)
- **관련 유스케이스**: [UC-USER-01](user-stories.md#uc-user-01-사용자-정보-조회)

**FR-USER-02: 인증 제한**
- **인증 방식**: Master Token (임시)
- **처리 과정**:
  1. Authorization 헤더에서 토큰 추출
  2. 토큰 유효성 검증
  3. 유효하지 않은 경우 401 에러 반환
- **적용 범위**:
  - 장바구니 관련 모든 API
  - 주문 및 결제 관련 모든 API
  - 쿠폰 발급 및 조회 API
...
- **공개 API** (인증 불필요):
  - 상품 목록/상세 조회
  - 인기 상품 조회
- **비고**: 실제 인증 구현(JWT 등)은 추후 Auth 도메인에서 별도 개발

#### 비기능 요구사항

**NFR-USER-PERF: 성능**
- 사용자 정보 조회: 평균 100ms 이내 응답

**NFR-USER-RELI: 안정성**
- **정보 보안**: 사용자는 본인의 정보만 조회 가능
- **토큰 검증**: 유효하지 않은 토큰은 즉시 거부
...
- **확인 조건**: 요청 수량 ≤ availableQuantity (가용 재고)
- **확인 시점**:
  - 장바구니 추가 시 (FR-CART-01)
  - 장바구니 수량 변경 시 (FR-CART-02)
  - 주문 생성 시 재고 예약 (FR-ORDER-02)
- **실패 처리**:
  - 재고 부족 시 에러 반환
...
- [FR-ORDER-02: 재고 확인 및 예약](#fr-order-02-재고-확인-및-예약)
...
| **FR-ORDER-02** | 주문 | 재고 확인 및 예약 | MUST | [UC-ORDER-01](user-stories.md) | TBD | ⏳ |
...
| **FR-USER-01** | 사용자 | 사용자 정보 조회 | MUST | [UC-USER-01](user-stories.md) | TBD | ⏳ |
| **FR-USER-02** | 사용자 | 인증 제한 | MUST | [UC-USER-01](user-stories.md) | TBD | ⏳ |
...
- 1.1.0 (2025-11-02): 피드백 반영 - 재고 정책 및 옵션 구조 간소화
  - 재고 "확보" → "예약"으로 용어 변경
  - 재고 분리 정책 추가 (available/reserved/sold)
  - OrderItem 기반 예약 관리로 변경
  - ProductOption type 필드 기반 그룹화
