...
9. [EPIC-6: 사용자 관리](#epic-6-사용자-관리)
10. [사용자 여정](#사용자-여정)
11. [사용자 스토리 추적 매트릭스](#사용자-스토리-추적-매트릭스)
...
| **EPIC-6: 사용자 관리** | 사용자 정보 조회 및 인증 관리 기능 | MUST |
...
graph TB
    ...
    EPIC6 --> US51[US-USER-01<br/>사용자 정보 조회]
...
고객이 자신의 프로필 정보를 조회하고 인증을 관리할 수 있도록 지원하는 기능군입니다.
...
**As a** 인증된 고객
**I want to** 내 프로필 정보를 확인하고 싶다
**So that** 내 계정 정보를 조회할 수 있다

#### 인수 조건 (Acceptance Criteria)

**정상 케이스:**
- [ ] **Given** 유효한 인증 토큰(Master Token)을 가지고 있을 때, **When** 사용자 정보 조회 요청 시, **Then** 내 프로필 정보를 받는다
- [ ] **Given** 프로필 정보에는, **Then** 사용자 ID, 이름, 이메일, 생성일이 포함된다
- [ ] **Given** 인증된 사용자는, **Then** 본인의 정보만 조회할 수 있다
- [ ] **Given** 조회 응답 시간은, **Then** 평균 100ms 이내여야 한다

**에러 케이스:**
- [ ] **Given** 인증 토큰이 없을 때, **When** 사용자 정보 조회 시, **Then** "토큰이 필요합니다" (401 Unauthorized)
- [ ] **Given** 유효하지 않은 토큰일 때, **When** 사용자 정보 조회 시, **Then** "유효하지 않은 토큰입니다" (401 Unauthorized)
- [ ] **Given** 사용자가 존재하지 않을 때, **When** 사용자 정보 조회 시, **Then** "사용자를 찾을 수 없습니다" (404 Not Found)
...
|------|---------|------|
| **성능** | 평균 응답 시간 | < 100ms |
| **보안** | 본인 정보만 조회 | 토큰 기반 인증 |
| **인증 방식** | Master Token (임시) | 테스트용 토큰 매핑 |

#### 비고

> **참고**: 현재 Dashboard 제품에서는 사용자 정보 조회만 제공됩니다. 사용자 목록 조회, 생성, 수정, 삭제 등의 관리 기능은 향후 Admin 제품에서 제공될 예정입니다.

> **인증**: Master Token 방식은 임시 테스트용이며, 향후 JWT 기반 인증으로 전환될 예정입니다.
...
| **US-USER-01** | 사용자 정보 조회 | EPIC-6 | 인증된 고객 | FR-USER-01, 02 | MUST | ⏳ 대기 |
