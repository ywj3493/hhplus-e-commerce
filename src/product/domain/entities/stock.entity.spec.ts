...
describe('Stock', () => {
  describe('생성', () => {
    ...
    it('productOptionId가 null인 Stock을 생성해야 함 (단일 옵션 상품)', () => {
      // Given
      const id = 'stock-1';
      const productId = 'product-1';
      const productOptionId = null;
      const totalQuantity = 100;
      ...
      // Then
      expect(stock.productId).toBe(productId);
      expect(stock.productOptionId).toBeNull();
    });
    ...
    it('음수 총 재고 수량에 대해 에러를 발생시켜야 함', () => {
      ...
      // When & Then
      expect(() =>
        Stock.create(id, productId, productOptionId, totalQuantity, 0, 0, 0),
    ...
    it('음수 가용 재고 수량에 대해 에러를 발생시켜야 함', () => {
      ...
      // When & Then
      expect(() =>
        Stock.create(id, productId, productOptionId, 100, -1, 0, 0),
  ...
  describe('초기화', () => {
    it('모든 수량이 가용 상태인 초기 재고를 생성해야 함', () => {
      ...
      // When
      const stock = Stock.initialize(id, productId, productOptionId, totalQuantity);
  ...
  describe('재고 예약', () => {
    it('재고를 성공적으로 예약해야 함', () => {
      // Given
      const stock = Stock.create('stock-1', 'product-1', 'option-1', 100, 100, 0, 0);
  ...
  describe('예약 복원', () => {
    it('예약된 재고를 성공적으로 복원해야 함', () => {
      // Given
      const stock = Stock.create('stock-1', 'product-1', 'option-1', 100, 80, 20, 0);
